# Description
This is a CSharp (C#) 🟪 to integrate queue storage 🧩 → 🪣📥 → 🧩. This is in order to study and practice for az204 certification exam 🤓📚🎓

# Made With
[![C#](https://img.shields.io/badge/c%23-8c39ac?style=for-the-badge&logo=data:image/svg+xml;base64,&logoColor=white&labelColor=000000)]()
[![NetCore & Net Framework](https://img.shields.io/badge/netcore%20&%20NET%20Framework-4f3ac9?style=for-the-badge&logo=dotnet&logoColor=white&labelColor=000000)]()
[![Azure](https://img.shields.io/badge/Azure-2b83cb?style=for-the-badge&logo=data:image/svg+xml;base64,&logoColor=white&labelColor=000000)]()

# Documentation

## Architecture Overview

This application demonstrates a simple queue-based messaging pattern using Azure Queue Storage. The same C# console application acts as both producer and consumer of messages.

```mermaid
graph LR
    A["C# Console Application<br/>(Producer)"]
    B[("Azure Queue Storage<br/>(Blob Storage)")]
    C["C# Console Application<br/>(Consumer)"]
    
    A -->|"Enqueue Messages"| B
    B -->|"Dequeue Messages"| C
    
    style A fill:#8c39ac,stroke:#333,stroke-width:2px,color:#fff
    style B fill:#2b83cb,stroke:#333,stroke-width:2px,color:#fff
    style C fill:#8c39ac,stroke:#333,stroke-width:2px,color:#fff
```

### Components

- **C# Console Application**: Sends messages to Azure Queue Storage and processes them from the queue
- **Azure Queue Storage**: Provides reliable message queuing service as part of Azure Blob Storage, enabling asynchronous communication

### Workflow

1. The application creates messages and enqueues them into Azure Queue Storage
2. Messages are reliably stored in the queue
3. The same application dequeues and processes the messages from the queue
